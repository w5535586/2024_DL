...
class ResidualAttentionModel_2Layer(nn.Module):
    def __init__(self):
        super(ResidualAttentionModel_2Layer, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True)
        ...
        self.mpool2 = nn.Sequential(
            nn.BatchNorm2d(512),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((1, 1))  # Use adaptive pooling to ensure the output size is 1x1
        )
